...
class PicamReadoutMeasure(Measurement):
    ...
    def setup(self):
        ...
        s.New(
            "spec_hw",
            str,
            initial="pi_spectrometer",
            choices=("pi_spectrometer", "acton_spectrometer"),
        )
    ...
    def update_background(self):
        print(
            self.name,
            "start updating background",
            5 * self.cam_hw.settings["ExposureTime"] / 1000,
            "s",
        )
    ...
    def run(self):
        ...
        while not self.interrupt_measurement_called:
            ...
            if s["wl_calib"] == "spectrometer":
                ...
                wls = hw.get_wl_calibration(
                    px_index, binning, m_order=1, pixel_width=cam_s["PixelWidth"]
                )
            else:
                s["x_axis"] = "pixels"
    ...
    def save_h5(self):
        self.new_dataset_metadata()
        h5_file = h5_io.h5_base_file(self.app, dataset_metadata=self.dataset_metadata)
    ...
    def save_png(self, fname=None):
        ...
        # ax2.imshow(
        #     image, aspect="auto", interpolation="none", cmap="viridis", origin="lower"
        # )
        ...
        ax.set_xlabel(self.settings["x_axis"])
        ax.set_ylabel("Intensity (a.u.)")

        if fname is None:
            path = self.dataset_metadata.get_file_path(".png")
        else:
            path = Path(self.app.settings["save_dir"]) / fname
    ...
    def setup_figure(self):
        ...
        file = str(this_path / "picam.svg")
        renderer = QtSvg.QSvgRenderer(file)
        svg_widget = QtSvgWidgets.QSvgWidget(file)
        svg_widget.setFixedSize(
            renderer.defaultSize().scaled(
                100, 100, QtCore.Qt.AspectRatioMode.KeepAspectRatio
            )
        )

        self.ui.cb_layout.insertWidget(0, svg_widget)
        ...
        self.ui.plot_layout.addWidget(self.dockarea)

        self.spec_plot = pg.PlotWidget()
        self.spec_plot_line = self.spec_plot.plot([1, 3, 2, 4, 3, 5])
        self.spec_plot.enableAutoRange()
        self.spec_widget = QtWidgets.QWidget()
        self.spec_layout = QtWidgets.QVBoxLayout(self.spec_widget)
        self.spec_layout.addWidget(self.spec_plot)
        self.spec_layout.addWidget(self.settings.New_UI(("x_axis",)))
        ...
        spec_dock = self.dockarea.addDock(
            name="Spec", position="below", widget=self.spec_widget
        )
        self.dockarea.addDock(
            name="Img",
            position="below",
            relativeTo=spec_dock,
            widget=self.img_graphlayout,
        )
        self.dockarea.addDock(
            name=self.cam_hw_name,
            position="below",
            relativeTo=spec_dock,
