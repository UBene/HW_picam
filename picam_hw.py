...
class PicamHW(HardwareComponent):
    ...
    def setup(self):
        ...
        s.New("target_serial_number", str, initial=OPEN_FIRST,
              description=f"if only one camera is connect set <i>{OPEN_FIRST}</i>. hit 'print available cameras' to see serial numbers")
        ...
        self.add_operation('print available cameras',
                           self.print_available_cameras)
    ...
    def connect(self):
        ...
        if self.cam.supports_rois:
            s.roi_w.change_min_max(1, s['ActiveWidth'])
            s.roi_x_bin.change_min_max(1, s['ActiveWidth'])
            s.roi_h.change_min_max(1, s['ActiveHeight'])
            s.roi_y_bin.change_min_max(1, s['ActiveWidth'])
            self.write_roi()
            self.cam.read_rois()
