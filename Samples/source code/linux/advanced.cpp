...
extern "C" const gchar _binary_advanced_extrinsic_dialog_ui_start[];
extern "C" const gchar _binary_advanced_extrinsic_dialog_ui_end[];
...
void CloseRepetitiveGateDialog(
    ...
    gint response_id,
    gpointer user_data );
void InitializeExtrinsicDialog();
void RefreshExtrinsicDialog();
void CloseExtrinsicDialog(
    GtkDialog* dialog,
    gint response_id,
    gpointer user_data );
...
PicamError PIL_CALL ParameterExtrinsicIntegerValueChanged(
    PicamHandle device,
    PicamParameter parameter,
    piint value );
PicamError PIL_CALL ParameterExtrinsicFloatingPointValueChanged(
    PicamHandle device,
    PicamParameter parameter,
    piflt value );
...
void LogExtrinsicValue( const std::string& message );
...
GtkBuilder* extrinsic_ = 0;             // - the extrinsic parameters dialog
...
// Shared State
////////////////////////////////////////////////////////////////////////////////
GMutex lock_;                           // - protects all shared state below
...
pibool WaitForAcquisitionCompleted()
{
    ...
    gint64 time = g_get_monotonic_time() + 10*G_USEC_PER_SEC;
    ...
    do
    {
        ...
        if( running )
            timedOut =
                !g_cond_wait_until( &acquisitionStatusChanged_, &lock_, time );
...
////////////////////////////////////////////////////////////////////////////////
// LogPostedExtrinsicValueChange
// - logs an extrinsic value change that was posted to the main thread
////////////////////////////////////////////////////////////////////////////////
gboolean LogPostedExtrinsicValueChange( gpointer user_data )
{
    std::string* message = static_cast<std::string*>( user_data );
    if( extrinsic_ )
        LogExtrinsicValue( *message );
    delete message;
...
////////////////////////////////////////////////////////////////////////////////
// PostExtrinsicValueChange
// - posts an extrinsic value change to display in the main thread
////////////////////////////////////////////////////////////////////////////////
void PostExtrinsicValueChange( const std::string& message )
{
    g_idle_add_full(
        G_PRIORITY_HIGH,
        LogPostedExtrinsicValueChange,
...
pibool InitializeImage()
{
    ...
    if( highResolution_ )
    {
        std::vector<pi16u>().swap( imageData16_ );
        imageData32_.resize( frameSize_ / sizeof( imageData32_[0] ) );
    }
    else
    {
        std::vector<pi32u>().swap( imageData32_ );
        imageData16_.resize( frameSize_ / sizeof( imageData16_[0] ) );
    }
    imageDataVersion_ = 0;
...
void GetBlackWhiteLevels(
    ...
    pi16u* white )
{
    // - generate a 16-bit histogram of image data intensities
...
////////////////////////////////////////////////////////////////////////////////
void GetBlackWhiteLevels(
    const std::vector<pi32u>& renderImageData,
    pi32u* black,
    pi32u* white )
{
    // - generate a 18-bit histogram of image data intensities
    static const pi32u histogramSize = 0x40000;
    static const pi32u histogramMax  = 0x3FFFF;
    static std::vector<piint> histogram( histogramSize );
    histogram.assign( histogram.size(), 0 );
    const piint dataPoints = static_cast<piint>( renderImageData.size() );
    for( piint i = 0; i < dataPoints; ++i )
        ++histogram[std::min( renderImageData[i], histogramMax )];

    // - find the start and end of the histogram
    pi32u b = 0, w = histogramMax;
    while( !histogram[b] || !histogram[w] )
    {
        if( !histogram[b] )
            ++b;
    ...
    if( b == w )
    {
        *black = b == 0x00000000 ? b   : b-1;
        *white = b == 0x00000000 ? b+1 : b;
        return;
    }
    ...
    std::vector<pibyte>* renderedImage )
{
    ...
    {
        // - determine linear interpolation black/white levels
        pi32u black = 0x00000000, white = 0x00000000;
        GetBlackWhiteLevels( renderImageData, &black, &white );
...
{
    std::vector<pi16u> renderImageData16;
    std::vector<pi32u> renderImageData32;
    pibool renderHighResolution = false;
    ...
    for( ;; )
    {
        ...
        {
            // - resize if necessary
            if( renderImageData32.size() != imageData32_.size() )
            {
                std::vector<pi16u>().swap( renderImageData16 );
                renderImageData32.resize( imageData32_.size() );
            }
            ...
            std::copy(
                imageData32_.begin(),
                imageData32_.end(),
                renderImageData32.begin() );
        }
        else
        {
            // - resize if necessary
            if( renderImageData16.size() != imageData16_.size() )
            {
                std::vector<pi32u>().swap( renderImageData32 );
                renderImageData16.resize( imageData16_.size() );
            }
...
{
    // - try to get available physical memory while accounting for the existing
    //   buffer
    piflt availableMemory = std::numeric_limits<piflt>::max();
    std::ifstream info( "/proc/meminfo" );
    const std::string memAvailable( "MemAvailable:" );
    std::string line;
    while( std::getline( info, line ) )
    {
        std::string::size_type pos = line.find( memAvailable );
        if( pos != std::string::npos )
        {
            pos += memAvailable.size();
            if( pos == line.size() )
                break;
            ...
            iss >> value;
            if( iss )
                availableMemory = value + buffer_.size();
            break;
        }
    }
    info.close();
    ...
    //   - try to contain at least 3 seconds worth of data without exceeding
    //     available physical memory
    //   - however, contain at least 2 readouts
    ...
    pi64s readouts = static_cast<pi64s>(
        std::max(
            std::min(
                std::ceil( 3. * onlineReadoutRate ),
                std::floor( availableMemory / readoutStride ) ),
            2. ) );
    calculatedBufferSize_ = static_cast<std::size_t>(
        readoutStride * readouts );
}
...
////////////////////////////////////////////////////////////////////////////////
pibool RegisterValueChangedCallback(
    PicamHandle model,
    PicamParameter parameter )
{
    // - get the value type
...
////////////////////////////////////////////////////////////////////////////////
// RegisterExtrinsicValueChangedCallback
// - initializes callback for any extrinsic parameter value changes from the
//   camera device
////////////////////////////////////////////////////////////////////////////////
pibool RegisterExtrinsicValueChangedCallback(
    PicamHandle device,
    PicamParameter parameter )
{
    // - get the value type
    PicamValueType valueType;
    PicamError error =
        Picam_GetParameterValueType( device, parameter, &valueType );
    if( error != PicamError_None )
    {
        DisplayError( "Failed to get parameter value type.", error );
        return false;
    ...
    // - register for extrinsic value changes
    switch( valueType )
    {
        case PicamValueType_Integer:
        case PicamValueType_Boolean:
        case PicamValueType_Enumeration:
            error =
                PicamAdvanced_RegisterForExtrinsicIntegerValueChanged(
                    device,
                    parameter,
                    ParameterExtrinsicIntegerValueChanged );
            break;
        case PicamValueType_FloatingPoint:
            error =
                PicamAdvanced_RegisterForExtrinsicFloatingPointValueChanged(
                    device,
                    parameter,
                    ParameterExtrinsicFloatingPointValueChanged );
            break;
        case PicamValueType_LargeInteger:
        case PicamValueType_Rois:
        case PicamValueType_Pulse:
    ...
    {
        DisplayError(
            "Failed to register for extrinsic value changes.",
            error );
        return false;
    }
...
void RegisterParameterCallbacks()
{
    ...
    for( piint i = 0; i < count; ++i )
    {
        ...
        // - register for value changes
        if( !RegisterValueChangedCallback( model, parameters[i] ) )
            continue;

        // - register for extrinsic value changes
        if( !RegisterExtrinsicValueChangedCallback( device_, parameters[i] ) )
            continue;
...
// - gets the selected camera device's extrinsic parameter value
////////////////////////////////////////////////////////////////////////////////
pibool GetExtrinsicParameterValue(
    PicamParameter parameter,
    std::string& formatted )
{
    // - clear string in case of errors
    formatted.clear();

    // - get the value type
    PicamValueType valueType;
    PicamError error =
        Picam_GetParameterValueType( device_, parameter, &valueType );
    if( error != PicamError_None )
    {
        DisplayError( "Failed to get parameter value type.", error );
        return false;
...
////////////////////////////////////////////////////////////////////////////////
// ParameterExtrinsicIntegerValueChanged
// - called when an integer parameter changes extrinsically
    ...
    piint value )
{
    ...
    PostExtrinsicValueChange( message );
...
////////////////////////////////////////////////////////////////////////////////
// ParameterExtrinsicFloatingPointValueChanged
// - called when a floating point parameter changes extrinsically
//   change
// - called on another thread
////////////////////////////////////////////////////////////////////////////////
PicamError PIL_CALL ParameterExtrinsicFloatingPointValueChanged(
    ...
    const PicamAcquisitionStatus* status )
{
    if( available && available->readout_count )
    {
        ...
        if( highResolution_ )
            std::memcpy( &imageData32_[0], frame, frameSize_ );
        else
            std::memcpy( &imageData16_[0], frame, frameSize_ );
        ++imageDataVersion_;
        g_cond_signal( &imageDataAvailable_ );
        al.Release();
...
pibool Initialize()
{
    ...
    // - initialize state
    // - note render thread runs until program termination so no need to unref
    g_mutex_init( &lock_ );
    g_cond_init( &acquisitionStatusChanged_ );
    g_cond_init( &imageDataAvailable_ );
    GError* threadError = 0;
    GThread* thread =
        g_thread_try_new( "Render", RenderThread, 0, &threadError );
    if( !thread )
    {
...
////////////////////////////////////////////////////////////////////////////////
// WatchExtrinsicParameters
// - shows the modeless extrinsic parameters dialog
////////////////////////////////////////////////////////////////////////////////
void WatchExtrinsicParameters()
{
    // - if dialog already open, bring it to the front
    if( extrinsic_ )
    {
        gtk_window_present(
            GTK_WINDOW(gtk_builder_get_object( extrinsic_, "dialog" )) );
        return;
    }

    // - create and show the dialog
    InitializeExtrinsicDialog();
}
...
void SaveFrame()
{
    ...
    piint pixelBitDepth =
        highResolution_
            ...
            : 16;
    file.write(
        reinterpret_cast<pichar*>( &pixelBitDepth ),
        sizeof( pixelBitDepth ) );

    // - write the image data
    if( highResolution_ )
        file.write(
            reinterpret_cast<pichar*>( &imageData32_[0] ),
            imageData32_.size() * sizeof( imageData32_[0] ) );
    else
        file.write(
            reinterpret_cast<pichar*>( &imageData16_[0] ),
            imageData16_.size() * sizeof( imageData16_[0] ) );
}
...
////////////////////////////////////////////////////////////////////////////////
// InitializeExtrinsicDialog
// - initializes and shows the extrinsic parameters dialog
////////////////////////////////////////////////////////////////////////////////
void InitializeExtrinsicDialog()
{
    // - create the dialog
    extrinsic_ =
        LoadDialog(
            _binary_advanced_extrinsic_dialog_ui_start,
            _binary_advanced_extrinsic_dialog_ui_end );
    if( !extrinsic_ )
        return;

    // - reflect current extrinsic values
    RefreshExtrinsicDialog();

    // - connect all signal handlers
    GtkDialog* dialog =
        GTK_DIALOG(gtk_builder_get_object( extrinsic_, "dialog" ));
    g_signal_connect(
        dialog,
        "response",
        G_CALLBACK(CloseExtrinsicDialog),
        0 );
...
////////////////////////////////////////////////////////////////////////////////
// RefreshExtrinsicDialog
// - refreshes the initial extrinsic parameter values in the extrinsic
//   parameters dialog
////////////////////////////////////////////////////////////////////////////////
void RefreshExtrinsicDialog()
{
    ...
    // - display the value of each parameter if it is extrinsic
    const PicamParameter* parameters;
    piint count;
    PicamError error = Picam_GetParameters( device_, &parameters, &count );
    if( error != PicamError_None )
    {
        DisplayError( "Failed to get camera parameters.", error );
        return;
    ...
    for( piint i = 0; i < count; ++i )
    {
        // - consider only values that change extrinsicly
        ...
        if( error != PicamError_None )
        {
            DisplayError( "Failed to get extrinsic dynamics.", error );
            continue;
        }
        if( !(extrinsic & PicamDynamicsMask_Value) )
            continue;

        // - log the current value
        std::string formatted;
        if( GetExtrinsicParameterValue( parameters[i], formatted ) )
        {
            std::string message =
                GetEnumString( PicamEnumeratedType_Parameter, parameters[i] ) +
                " value is " + formatted;
...
////////////////////////////////////////////////////////////////////////////////
// LogExtrinsicValue
// - appends a message to the extrinsic parameters dialog value log
////////////////////////////////////////////////////////////////////////////////
void LogExtrinsicValue( const std::string& message )
{
    // - move cursor to the beginning
...
////////////////////////////////////////////////////////////////////////////////
// CloseExtrinsicDialog
// - handles closing of the extrinsic parameters dialog
////////////////////////////////////////////////////////////////////////////////
void CloseExtrinsicDialog(
    GtkDialog* dialog,
    gint /*response_id*/,
    gpointer /*user_data*/ )
{
    // - clean up
    gtk_widget_destroy( GTK_WIDGET(dialog) );
    g_object_unref( G_OBJECT(extrinsic_) );
    extrinsic_ = 0;
}
...
void ProcessMenuAction( GtkAction* action, gpointer /*user_data*/ )
{
    ...
    if( name == "save_frame" )
        ...
        SetRepetitiveGate();
    else if( name == "watch_extrinsic_parameters" )
        WatchExtrinsicParameters();
