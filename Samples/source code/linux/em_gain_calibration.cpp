...
bool UpdateTemperatureSetPoint()
{
    ...
    if (errCode == PicamError_None)
    {
        ...
        {
            gchar tempRange[64];	
            sprintf(tempRange, "Temperature (%.f to %.f)", (double)tempConstraint->minimum, (double)tempConstraint->maximum);
            gtk_frame_set_label(
...
bool UpdateCalibrationDate()
{
    ...
    {
        gchar buffer[32];	
        sprintf(buffer, "%02d/%02d/%04d",(int)date.month,(int) date.day, (int)date.year);
        gtk_label_set_text(
...
bool UpdateCurrentTemperature()
{
    ...
    {    
        gchar buffer[32];
        sprintf(buffer, "%.f", (double)currentTemp);
        gtk_label_set_text(
...
{	        
    ...
    gtk_button_set_label(
        GTK_BUTTON(gtk_builder_get_object(ui_, "calibrate_button")),
        "Abort");
    calibrationThread_ = g_thread_new("Calibration", CalibrationThread, NULL);
}
////////////////////////////////////////////////////////////////////////////////
void EndCalibration()
{
