...
enum
{
    ...
    WM_LOG_EXTRINSIC_VALUE_CHANGE   // - log extrinsic update (wParam: wstring*)
};
...
INT_PTR CALLBACK RepetitiveGateDialogProc(
    ...
    WPARAM wParam,
    LPARAM lParam );
void RefreshExtrinsicDialog( HWND dialog );
INT_PTR CALLBACK ExtrinsicDialogProc(
    HWND hwndDlg,
    UINT uMsg,
    WPARAM wParam,
    LPARAM lParam );
...
HWND extrinsic_ = 0;                    // - the extrinsic parameters dialog
...
////////////////////////////////////////////////////////////////////////////////
// NarrowChar
// - helper to perform narrowing conversion from wstring to string
////////////////////////////////////////////////////////////////////////////////
struct NarrowChar
{
...
void CalculateBufferSize( piint readoutStride, piflt onlineReadoutRate )
{
    ...
    if( ::GlobalMemoryStatusEx( &status ) )
        availableMemory = static_cast<piflt>(
            status.ullAvailPhys + buffer_.size() );
    ...
    PicamParameter parameter )
{
    ...
    {
        DisplayError(
            L"Failed to register for extrinsic value changes.",
            error );
        return false;
    }
...
void OpenCamera( const PicamCameraID& id )
{
    ...
    if( device_ )
    {
        ...
        if( extrinsic_ )
            RefreshExtrinsicDialog( extrinsic_ );
    }
}
    ...
    piint value )
{
    ...
    PostMessage(
        main_,
        WM_LOG_EXTRINSIC_VALUE_CHANGE,
        reinterpret_cast<WPARAM>( new std::wstring( message ) ),
        0 );
...
void WatchExtrinsicParameters()
{
    ...
    if( extrinsic_ )
    {
        SetForegroundWindow( extrinsic_ );
        return;
    }
...
////////////////////////////////////////////////////////////////////////////////
void LogExtrinsicValue( HWND dialog, const std::wstring& message )
{
    // - move caret to the beginning
    SendDlgItemMessage(
        dialog,
...
{
    // - clear the edit control
    SetDlgItemText( dialog, IDC_VALUES, 0 );
    ...
    for( piint i = 0; i < count; ++i )
    {
        ...
        if( error != PicamError_None )
        {
            DisplayError( L"Failed to get extrinsic dynamics.", error );
            continue;
        }
        ...
        if( GetExtrinsicParameterValue( parameters[i], formatted ) )
        {
            std::wstring message =
                GetEnumString( PicamEnumeratedType_Parameter, parameters[i] ) +
                L" value is " + formatted;
            LogExtrinsicValue( dialog, message );
        }
    }
    Picam_DestroyParameters( parameters );
}
...
////////////////////////////////////////////////////////////////////////////////
void CloseExtrinsicDialog( HWND dialog )
{
    EndDialog( dialog, 0 );
    extrinsic_ = 0;
}
...
////////////////////////////////////////////////////////////////////////////////
// ExtrinsicDialogProc
// - extrinsic parameters dialog window procedure
////////////////////////////////////////////////////////////////////////////////
INT_PTR CALLBACK ExtrinsicDialogProc(
    HWND hwndDlg,
    UINT uMsg,
    ...
    LPARAM lParam )
{
    ...
    {
        case WM_INITDIALOG:
            RefreshExtrinsicDialog( hwndDlg );
            break;
        case WM_CLOSE:
            wParam = IDCANCEL;
            // - fall through
...
void ConnectDemoCamera( HWND dialog )
{
    ...
    std::string serialNumber( wideSerialNumber.length(), '\0' );
    std::transform(
...
void ProcessMenuCommand( piint id )
{
    switch( id )
    {
        ...
        case ID_CONFIGURE_SETREPETITIVEGATE:
            SetRepetitiveGate();
            break;
        case ID_CONFIGURE_WATCHEXTRINSICPARAMETERS:
            WatchExtrinsicParameters();
            break;
    ...
    __in  LPARAM lParam )
{
    switch( uMsg )
    {
        ...
        case WM_ACQUISITION_STOPPED:
            acquiring_ = false;
            break;
        case WM_LOG_EXTRINSIC_VALUE_CHANGE:
        {
            std::wstring* message = reinterpret_cast<std::wstring*>( wParam );
            if( extrinsic_ )
                LogExtrinsicValue( extrinsic_, *message );
            delete message;
            break;
        }
        default:
...
//##############################################################################
////////////////////////////////////////////////////////////////////////////////
// IsModelessDialogMessage
// - returns true if the window message belongs to a modeless dialog
////////////////////////////////////////////////////////////////////////////////
pibool IsModelessDialogMessage( MSG& msg )
{
    // - check against all modeless dialog
    const HWND modeless[] =
    {
        exposure_,
        repetitiveGate_,
    ...
    const piint count = sizeof( modeless ) / sizeof( *modeless );
    for( piint i = 0; i < count; ++i )
        if( modeless[i] && IsDialogMessage( modeless[i], &msg ) )
            return true;
...
piint RunMessageLoop()
{
    ...
    for( ;; )
    {
        ...
        else if( !IsModelessDialogMessage( msg ) )
        {
            // - process message
