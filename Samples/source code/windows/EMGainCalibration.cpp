...
bool UpdateTemperatureSetPoint()
{
    ...
    {
        WCHAR buffer[32];	
        swprintf(buffer, 32, L"%.f", (double)temp);
        ...
        {
            WCHAR tempRange[64];	
            swprintf(tempRange, 64, L"Temperature (%.f to %.f)", (double)tempConstraint->minimum, (double)tempConstraint->maximum);
...
bool UpdateCalibrationDate()
{
    ...
    {
        WCHAR buffer[32];	
        swprintf(buffer, 32, L"%02d/%02d/%04d", (int)date.month,(int) date.day, (int)date.year);
...
bool UpdateCurrentTemperature()
{
    ...
    {    
        WCHAR buffer[32];
        swprintf(buffer, 32, L"%.f",(double)currentTemp);
